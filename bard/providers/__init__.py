...
def get_provider_from_config(provider_type, provider_config):
    ...
    if provider_config["name"] not in registered_providers:
        raise Exception(
            "Invalid Provider {}: unknown provider type {}".format(
...
class DownloadAggregator(object):
    def __init__(self, config):
        ...
        for source in config.get("sources", []):
            ...
            self._providers[source["name"]] = get_provider_from_config(
                "download", source
            )
    ...
    def _get(self, provider_name):
        if provider_name not in self._providers:
            raise Exception(
                "Referenced provider hasn't been configured as a source: {}".format(
                    provider_name
                )
            )
    ...
    def search(self, episode):
        provider = self._get(
            episode.series.download_provider or self._config["default"]
        )
...
class Providers(object):
    def load_from_config(self, config):
        ...
        self.fetch = get_provider_from_config(
            "fetch", config["providers"]["fetch"]["source"]
        )
        self.info = InfoAggregator(config["providers"]["info"])
        self.library = get_provider_from_config(
            "library", config["providers"]["library"]["source"]
        )
        self.notify = get_provider_from_config(
            "notify", config["providers"]["notify"]["source"]
        )
