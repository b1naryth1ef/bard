...
SUPPORTED_EXTERNAL_SERIVCES = OrderedDict(
    [("imdb", "imdb"), ("tvdb", "thetvdb"), ("tvrage", "tvrage")]
)

STATUS_MAP = {"Running": Series.AirStatus.CONTINUING, "Ended": Series.AirStatus.ENDED}
...
class TVMazeInfoProvider(object):
    ...
    def _cast_season(obj):
        return SeasonMetadata(
            number=str(obj["number"]), episode_count=obj["episodeOrder"]
        )
    ...
    def _cast_episode(obj):
        ...
        if obj["airstamp"]:
            airdate = datetime.strptime(
                obj["airstamp"].split("+")[0], "%Y-%m-%dT%H:%M:%S"
            )
    ...
    def search_series(self, name):
        results = self.get("search/shows", params={"q": name})
    ...
    def find_by_external(self, external_ids):
        for service, service_key in SUPPORTED_EXTERNAL_SERIVCES.items():
            ...
            try:
                result = self.get(
                    "lookup/shows", params={service_key: external_ids[service]}
                )
    ...
    def get_episodes(self, series_id, season):
        ...
        return [
            self._cast_episode(i) for i in results if str(i["season"]) == str(season)
        ]
