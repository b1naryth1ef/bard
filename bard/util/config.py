...
class Configuration(object):
    ...
    def load(self):
        for location in self._locations:
            ...
            raise ConfigurationException(
                "Failed to find configuration (paths: {})".format(
    ...
    def _validate(self):
        for path, value in self._validations.items():
            ...
            if data is None:
                raise ConfigurationException(
                    "Required value `{}` unset in configuration".format(path)
                )

            if value is not None:
                if not isinstance(data, value):
                    raise ConfigurationException(
                        "Invalid type for value `{}` is {} should be {}".format(
                            path, type(data), value
                        )
                    )
