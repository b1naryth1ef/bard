...
__all__ = ["JSONField", "BaseModel", "database", "REGISTERED_MODELS", "init_db"]
...
def init_db(config):
    for file_name in os.listdir(os.path.dirname(os.path.abspath(__file__))):
        ...
        __import__(
            "bard.models.{}".format(os.path.splitext(os.path.basename(file_name))[0])
        )
    ...
    if obj.scheme == "sqlite":
        database.initialize(
            SqliteExtDatabase(
                obj.netloc, pragmas=[("foreign_keys", 1)], check_same_thread=False
            )
        )
    else:
        raise Exception(
            "Unsupported database adapter `{}`, for DB url `{}`".format(
                obj.scheme, config["database"]
            )
        )
