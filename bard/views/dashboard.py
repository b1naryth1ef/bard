...
def _dashboard_args():
    return {
        ...
        "recently_aired_episodes": Episode.select()
        .join(Season)
        .join(Series)
        .where(
        ...
        .order_by(Episode.airdate.desc())
        .limit(25),
        "upcoming_episodes": Episode.select()
        .join(Season)
        .join(Series)
        .where(
            (Series.subscribed >> True)
        ...
        .order_by(Episode.airdate.asc())
        .limit(25),
        "active_torrents": Torrent.select()
        .where(
        ...
        "stalled_episodes": Episode.select()
        .join(Torrent)
        .where(
            (
                Torrent.state
                << (int(Torrent.State.SEEDING), int(Torrent.State.COMPLETED))
            )
...
def dashboard_index():
    ...
    return render_template("dashboard/index.html", parts=parts, **_dashboard_args())
